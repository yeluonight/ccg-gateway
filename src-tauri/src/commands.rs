...
#[tauri::command]
pub async fn import_from_local(data: Vec<u8>, app: tauri::AppHandle) -> Result<()> {
    let db_path = get_data_dir().join("ccg_gateway.db");
    ...
    tracing::info!("数据库导入成功，准备重启应用...");

    // 延迟 500ms 后重启，给前端时间处理响应
...
pub async fn list_webdav_backups(db: State<'_, SqlitePool>) -> Result<Vec<WebdavBackup>> {
    ...
    // Simple parsing - look for .db files
    for line in body.lines() {
        if line.contains(".db") {
            // Extract filename from href
            if let Some(start) = line.find("ccg_gateway_") {
                if let Some(end) = line[start..].find(".db") {
                    let filename = format!("{}.db", &line[start..start + end]);
